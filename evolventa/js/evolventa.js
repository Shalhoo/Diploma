...
//
        ...
        ruler.onmousedown = function(event) {
            var shiftX = event.clientX - ruler.getBoundingClientRect().left,
            shiftY = event.clientY - ruler.getBoundingClientRect().top
        ...
        blankSA.onmousedown = function(event) {
            var gearCopyArray = [], blankS = document.createElement('g'), blankArray = [] // kik to gearCopyArray
...
//
            blankS.onmouseup = function(event) {
                function onMouseMove(event) {
                    ...
                    creteCircle = elemBelow.closest('.machine-fullStand'),
                    standShelter = elemBelow.closest('.toolsStand-shelter')
...
//
               function drawLine() {
                   ...
                   /*function fourthParam() {
                        for (var i = 1; i < 20; i++) {
                           var fourthParam = fourth * i;
                        }
                        return fourthParam
                    }

                    var foParam = fourthParam() */
...
//
                function change() {
                    lever.onmousedown = () => {
                        ...
                        document.getElementById("lever").style.transformOrigin = "15% 85%"
                         ...
                         for (let pair of lineStyle.entries()) {
                            if (pair[0] == mParam) {
                                secPar = pair[1]
                                lineStyleLeft(313, secPar)
                            }
                        }
                    }
                    ...
                    pedal.onmouseup = function() {
                        var gear = document.createElement('g'), angle = 10, // si to gear
                        ...
                        gear.innerHTML = `<svg width="313" height="300"  xmlns="http://www.w3.org/2000/svg" id="ggg"  clip-path="url(#cut-off-bottom)" class = "ggg">
                        <path d="
                    ...
                    L${shiftX2 + sixth * 65 + fourth * 19},${secYParam} L${shiftX2 + sixth * 65 + fourth * 19},${shiftY2}" style="stroke:black; stroke-width:1; fill:green;""/>

                    </svg>`

                        document.body.append(gear)
                        gear.style.position = 'absolute'
                        gear.style.left = 258 + 'px'
                        gear.style.top = 170 + 'px'
                        gear.style.zIndex = 101
                        gear.style.pointerEvents = 'none'
                        blankS.style.transform = 300
                        rotateSin(angle)
                        gearCopyArray.push(gear)
                        console.log(gearCopyArray)
                        document.getElementById("pedal").style.transform = "rotate(360deg)"
                    }

                    function rotateSin(angle) {
                        for (var n = gearCopyArray.length - 1; n >= 0; n--) {
                            gearCopyArray[n].style.transformOrigin = "156.9px 123px"
                            gearCopyArray[n].style.transform = "rotate(" + angle + "deg)"
                            angle = ((angle + 5))
                    ...
                    pedal.onmousedown = () => {
                       ...
                       for (let pair of lineStyle.entries()) {
                            if (mParam == 23) {
                                lineStyleLeft(1011.08, 120.5)
                            } else if (pair[0] == mParam) {
                                parB = pair[1]
                                lineStyleLeft(parA, parB)
                            }
                        }
                    }
                }
                ...
                function CreateCircle() {
                    fullStand.onmouseup = function() {
                            ...
                            function openSin() {
                                for (var n = gearCopyArray.length - 1; n >= 0; n--) {
                                    // gearCopyArray[n].style.transformOrigin = ((dParam / 2)+57) + "px" + " " + ((dParam / 2)+24) + "px"
                                    gearCopyArray[n].style.opacity = 1
                                }
                            }

                            blankS.onmouseup = function() {
                                if (gearCopyArray.length == 0) {
                                    document.removeEventListener('mousemove', onMouseMove)
                                    blankS.onmouseup = null
                                    blankS.innerHTML = `<svg width="42" height="51" viewBox="0 0 42 51" fill="none" xmlns="http://www.w3.org/2000/svg">
                                        <mask id="path-1-inside-1_4_16" fill="white">
                                    ...
                                    function deleteSin() {
                                        for (var n = gearCopyArray.length - 1; n >= 0; n--) {
                                            gearCopyArray[n].style.opacity = 0
                                        }
                                    }
